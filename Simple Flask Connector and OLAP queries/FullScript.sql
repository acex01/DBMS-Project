...
CREATE TABLE IF NOT EXISTS `mydb`.`customer` (
  ...
  `rating` DOUBLE NULL DEFAULT NULL,
  `Trips_Trip_ID` DOUBLE NOT NULL,
  `Drivers_D_Phone_number` DOUBLE NOT NULL,
  `Drivers_D_Phone_number1` DOUBLE NOT NULL,
  `Car_number` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`C_Phone_Number`, `C_email_ID`, `Trips_Trip_ID`, `Drivers_D_Phone_number`, `Drivers_D_Phone_number1`, `Car_number`),
  UNIQUE INDEX `Phone Number_UNIQUE` (`C_Phone_Number` ASC) VISIBLE,
  UNIQUE INDEX `email number_UNIQUE` (`C_email_ID` ASC) VISIBLE,
